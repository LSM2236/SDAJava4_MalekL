...
public class Main {
	...
	static String getTopStudentName(Student studentArray[], Result resultArray[]) {
		...
		Result najlepszyWynik = resultArray[0];
		for (Result result : resultArray) {
			if (result.score > najlepszyWynik.score){
				najlepszyWynik = result;
			}
		}
		for (Student studencik : studentArray)
			if (najlepszyWynik.studentId == studencik.studentId){
				topStudentName = studencik.name;
				break;
			}
